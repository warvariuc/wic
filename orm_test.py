...
print('\nUPDATE query:')
print(db._update(Books.name('_' + book.name), Books.price(Books.price + 1),
                 where=(Books.id == book.id)))
db.update(Books.name('A new title with raised price'), Books.price(Books.price + 1),
          where=(Books.id == book.id))
...
print('\nSelecting one book with id=1:\n ',
      db.select('*', from_=[Books, orm.Join(Authors, Books.author == Authors.id)],
                where=(Books.id == 1)))
