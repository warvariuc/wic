...
class Query(object):
    ...
    def __init__(self, db, op, first=None, second=None):
        self.db = self._db = db
        self.op = op
        self.first = first
        self.second = second
...
class Books(Table):
    ...
    price = DecimalField(decimalPlaces=2) # 2 decimal places



dbAdapters = [SqliteAdapter, MysqlAdapter, PostgresqlAdapter] # available adapters
...
def connect(uri):
    for dbAdapterClass in dbAdapters: # search for suitable adapter by protocol
        ...
        if uri.startswith(uriStart):
            ...
            defaultDbAdapter = dbAdapterClass(uri[len(uriStart):])



if __name__ == '__main__':
    connect('sqlite://conf/databases/test.sqlite')

    book = Books.new(name='Just for Fun: The Story of an Accidental Revolutionary',
                     price=Decimal('14.99')) # new item in books catalog 
    ...
    books = Books((Books.id >= bookId) & (Books.price >= Decimal('0.01'))).select()
    print(books[0])
