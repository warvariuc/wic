...
class OptionsForm(QDialog):
    ...
    def __init__(self, parent=None):
        ...
        resourceModuleNamesGroupBox = QGroupBox("Resource Module Names")
    ...
    def accept(self):
        ...
        settings.setValue("qrc_resources", "1" if self.qrcRadioButton.isChecked() else "0")
        settings.setValue("pyuic4x", "1" if self.pyuic4xCheckBox.isChecked() else "0")
        super().accept()


    def setPath(self, tool):
        if tool == "pyuic4": label = self.pyuic4Label
        elif tool == "pyrcc4": label = self.pyrcc4Label
        elif tool == "pylupdate4": label = self.pylupdate4Label
        elif tool == "lrelease": label = self.lreleaseLabel
        path = QFileDialog.getOpenFileName(self,
                "Make PyQt - Set Tool Path", label.text())
        if path: label.setText(QDir.toNativeSeparators(path))
...
class Form(QMainWindow):
    ...
    def __init__(self):
        ...
        rememberPath = None
        try:
            rememberPath = bool(int(settings.value("rememberpath", 1 if Windows else 0)))
        except ValueError:
    ...
    def closeEvent(self, event):
        settings = QSettings()
        settings.setValue("rememberpath", int(self.rememberPathAction.isChecked()))
